Dummy::Application.configure do
  ...
  if defined?(Prosopite)
    ...
    # Both halves of this differ by adapter, and getting either wrong makes
    # the guard silently report a single record's two associations as an
    # N+1: identifiers are double-quoted on PostgreSQL and SQLite but
    # backticked on MySQL, and the bind marker is `?` on SQLite and MySQL
    # but `$1` on PostgreSQL.
    SINGLE_ROW_LOOKUP = /WHERE .*["`](?:id|record_id)["`] = (?:\?|\$\d+)/i
    SPREE_LOOKUP_BINDS = Hash.new { |hash, key| hash[key] = [] }

    ActiveSupport::Notifications.subscribe('sql.active_record') do |_, _, _, _, data|
      sql = data[:sql].to_s
      next unless SINGLE_ROW_LOOKUP.match?(sql)

      values = Array(data[:binds]).map { |bind| bind.respond_to?(:value) ? bind.value : bind }
      SPREE_LOOKUP_BINDS[sql] << values
    end

    Prosopite.singleton_class.prepend(Module.new do
      def fingerprint(query)
        return super unless SINGLE_ROW_LOOKUP.match?(query)

        # Consume the ids in the order they were queried, so two lookups of
        # different rows fingerprint apart and two of the same row together.
        recorded = SPREE_LOOKUP_BINDS[query]
        "#{super} /* row=#{recorded.shift.inspect} */"
      end
    end)

    RSpec.configure { |config| config.before { SPREE_LOOKUP_BINDS.clear } } if defined?(RSpec)
  end
end
