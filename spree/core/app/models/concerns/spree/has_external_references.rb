module Spree
  ...
  module HasExternalReferences
    ...
    # Replaces the record's references with the given list, leaving systems the
    # payload does not mention untouched. This is what the Admin API's
    # `external_references: [{ system:, external_id: }]` parameter writes: a
    # connector states its own key without disturbing another system's.
    #
    # @param references [Array<Hash>] entries with :system and :external_id
    # @return [void]
