...
it accounted for more than half of the first run's failures. The second is two associations on one table — an
order's billing and shipping address, a category's image and square image.
Those normalize together no matter what, so one record reads as a repeat
while the real cost is one query per association, not per record. The guard
separates them by recording which row each single-row lookup asked for: two
associations fetch different rows and stay apart, while the same row
fetched once per record still reports. Ignoring such lookups wholesale is
not an option — it blinds the guard to the commonest N+1 there is.
